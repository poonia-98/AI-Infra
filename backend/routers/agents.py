...
async def start_agent(agent_id: uuid.UUID, db: AsyncSession = Depends(get_db)):
    result = await agent_service.start_agent(db, agent_id)
...
async def stop_agent(agent_id: uuid.UUID, db: AsyncSession = Depends(get_db)):
    result = await agent_service.stop_agent(db, agent_id)
    if not result.get("success"):
...
async def restart_agent(agent_id: uuid.UUID, db: AsyncSession = Depends(get_db)):
    result = await agent_service.restart_agent(db, agent_id)
    if not result.get("success"):
